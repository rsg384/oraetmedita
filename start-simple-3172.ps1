# Servidor Simples na Porta 3172
# Projeto: Ora et Medita

Write-Host "Servidor Simples - Porta 3172" -ForegroundColor Green
Write-Host "Projeto: Ora et Medita" -ForegroundColor Yellow
Write-Host ""

# Verificar se a porta está livre
Write-Host "Verificando porta 3172..." -ForegroundColor Cyan
$portInUse = Get-NetTCPConnection -LocalPort 3172 -ErrorAction SilentlyContinue

if ($portInUse) {
    Write-Host "Porta 3172 ja esta em uso!" -ForegroundColor Red
    Write-Host "Encerre o processo ou use outra porta." -ForegroundColor Yellow
    exit 1
} else {
    Write-Host "Porta 3172 esta livre!" -ForegroundColor Green
}

Write-Host ""
Write-Host "Iniciando servidor na porta 3172..." -ForegroundColor Green
...
try {
    ...
    $listener.Prefixes.Add("http://localhost:3172/")
    $listener.Start()
    
    Write-Host "Servidor iniciado com sucesso!" -ForegroundColor Green
    Write-Host "Acesse: http://localhost:3172" -ForegroundColor Blue
